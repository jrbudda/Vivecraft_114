...
public class MenuWorldRenderer {
	...
	public void prepare() {
		if (vertexBuffers == null) {
			...
			BlockLeaves.setRenderTranslucent(true);
		...
		if (this.lightmapUpdateNeeded)
		{
			...
			for (int i = 0; i < 16; ++i)
			{
				for (int j = 0; j < 16; ++j)
				{
					...
					float f18 = (float)this.mc.gameSettings.gammaSetting;
					float f13 = 1.0F - f9;
					float f14 = 1.0F - f10;
					float f15 = 1.0F - f11;
	...
	public boolean areEyesInFluid(Tag<Fluid> tagIn)
	{
		...
		return isFluidTagged(ifluidstate, tagIn) && pos.y < (double)((float)blockpos.getY() + ifluidstate.getHeight() + 0.11111111F);
	}
	...
	public static class MenuFogRenderer {
		...
		private void updateWaterFog(IWorldReaderBase worldIn)
		{
		...
		public void setupFog(int startCoords) {
			...
			/*else if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPotionActive(MobEffects.BLINDNESS))
			{
				...
				if (i < 20)
				{
					f3 = 5.0F + (this.entityRenderer.getFarPlaneDistance() - 5.0F) * (1.0F - (float)i / 20.0F);
				}
        ...
        public void applyFog(boolean blackIn)
        {
            if (blackIn)
            {
                GlStateManager.fogfv(GL11.GL_FOG_COLOR, this.blackBuffer);
            }
            else
            {
                GlStateManager.fogfv(GL11.GL_FOG_COLOR, this.getFogBuffer());
            }
        }
