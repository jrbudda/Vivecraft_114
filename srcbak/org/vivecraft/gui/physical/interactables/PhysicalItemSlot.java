...
public class PhysicalItemSlot implements Interactable {
	...
	public void render(double partialTicks, int renderLayer){
		...
		ItemRenderer renderItem = Minecraft.getMinecraft().getItemRenderer();
		...
		if(renderLayer==0) {
			...
			renderItem.renderItem(item, mc.player, ItemCameraTransforms.TransformType.FIXED, false);
		}
	...
	//		GlStateManager.multMatrixf(Convert.matrix(this.counterRot).toMCMatrix4f());
			...
			if (this.getOpacity() == 1) {
				EntityRenderer.drawText(mc.fontRenderer, "" + item.getCount(),
						0,0,0, 0, 0, true);
			}
		}
