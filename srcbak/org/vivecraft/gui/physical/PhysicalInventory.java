...
public class PhysicalInventory extends PhysicalItemSlotGui {
	...
	public PhysicalInventory(EntityPlayerSP player) {
		super(player);
		this.player = player;
	...
	public void onUpdate() {
		...
		int offhand = mc.gameSettings.mainHand == EnumHandSide.RIGHT ? 1 : 0;
	...
	public void postGuiChange(PhysicalGui gui) {
		...
		if (gui != null && gui.container != null)
			...
			this.container=player.inventoryContainer;
		hotbar.open(null);
		loadSlots();
		pickItemBackUp();
		mc.physicalGuiManager.guiTransitionOverride = null;
	...
	public void render(double partialTicks) {
		...
		for (int pass = 0; pass < 2; pass++) {
			...
			mc.renderGlobal.renderCustomModel(loc);
			GlStateManager.popMatrix();
	...
	public class Hotbar extends PhysicalItemSlotGui {
		...
		public Vec3d getAnchorPos(double partialTicks) {
			...
			if (data == null)
				data = mc.vrPlayer.vrdata_world_pre;
			//Vec3d prev=new Vec3d(mc.entityRenderer.rveprevX,mc.entityRenderer.rveprevY,mc.entityRenderer.rveprevZ);
			//Vec3d current=new Vec3d(mc.entityRenderer.rveX,mc.entityRenderer.rveY,mc.entityRenderer.rveZ);
